...
int ina22x_init(ina22x_t *dev, const ina22x_params_t *params)
{
    ...
    dev->alert_pin = params->alert_pin;

    //memcpy(dev, params, sizeof(ina22x_t));

    DEBUG("ina22x_init dev: %p, config: 0x%x\n",dev, params->config);

    if( (ina22x_write_config_reg(dev, params->config) == INA22X_OK) &&
        (ina22x_write_calibration_reg(dev, params->cal) == INA22X_OK) ){
            DEBUG("ina22x_init: INA22X_OK\n");
        return INA22X_OK;
    }

    DEBUG("ina22x_init: INA22X_I2C_ERR\n");

    return INA22X_I2C_ERR;
}

int ina226_activate_int(ina22x_t *dev, uint16_t me_config, gpio_t pin, gpio_cb_t callback, void *arg)
{
    ...
    if (ina22x_write_reg(dev, INA226_REG_MASK_ENABLE, me_config) == INA22X_OK ) {
        if ( gpio_init_int(pin, GPIO_IN_PU, GPIO_FALLING, callback, arg) == 0) {
            return INA22X_OK;
        }
